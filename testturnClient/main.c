...
int turnPort = 3478;
char *turnHost = "www.lymggylove.top";
char *turnUser = "lym";
...
void turnWriteUdp(turnMessage * m)
{
    turnWriteAttrInt(m, turnAttrRequestedTransport, 17<<24);
}
...
void turnWriteMessage(turnMessage * m, int action,int count)
{
    switch (action)
    {
        case turnActionBind:
    ...
    while ((offset - turnHeaderSize) < m->length)
    {
        ...
        switch (attr)
        {
            ...
            case turnAttrRealm:{
                ...
                //存储
                memcpy(turnRealmStr, m->data + m->ofs, len);
...
void turnRecv(turnAddress * a, uint8_t *data, long datalen)
{
    ...
    if (datalen < turnBufferSize)
    {
        ...
        turnParseMessage(&m);
        if(m.errorcode == 401){
            turnWriteMessage(&m, turnActionAllocate,1);
